 ...
 * @default http://worldtimeapi.org/api/America/Chicago
 *
 * @param JSON Property
 ...
 * ============
 *
 * Saves a Timestamp provided by http://worldtimeapi.org/ to calculate how much time has transpired.
...
(function() {
  ...
  var webApi = String(
    parameters["Web API"] || "http://worldtimeapi.org/api/America/Chicago"
  );
   ...
   * It'll save the last read value in the first position of the array, and store the new value in the second position of the array.
   * JS arrays are 0-index, RMMV Control Variables are 1-based.
   */
  AXLLib.saveTime = function(controlVariableIndex, show) {
    ...
    _request.onload = function() {
      if (_request.status < 400) {
        ...
        // if (show && AXLLib.isOnMap()) {
        //   $gameMessage.clear();
        //   $gameMessage.add(AXLLib.getTimeDifference());
