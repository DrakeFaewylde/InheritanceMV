...
(function() {
      ...
      if (hitIndex === this.index()) {
        ...
        if (this.isVolumeSymbol(symbol)) {
          ...
          if (x >= titleWidth && x <= titleWidth + statusWidth) {
            this.changeValue(
              symbol,
              Math.round(((x - titleWidth) / statusWidth) * 100)
            );
          }
   ...
   *
   */
  Window_Options.prototype.drawSlider = function(value, x, y, maxWidth) {
    var lineY = y + this.lineHeight() / 2;

    this.contents.paintOpacity = 100;

    this.contents.fillRect(x, lineY, maxWidth - 1, 2, this.normalColor());
